...
io.on("connection", (client) => {
  ...
  client.on("JOIN", (newRoom) =>
    isRoomFull(io, newRoom)
      ? fullRoom(io, client, newRoom)
      : (currentRoom = joinRoom(io, client, currentRoom, newRoom))
  );

  client.on("MESSAGE", (message) => sendMessage(client, currentRoom, message));

  client.on("PUBLIC_KEY", (publicKey) =>
    sendPublicKey(client, currentRoom, publicKey)
  );

  client.on("disconnect", () =>
    notifyRoomUserDisconnected(client, currentRoom)
  );
});
